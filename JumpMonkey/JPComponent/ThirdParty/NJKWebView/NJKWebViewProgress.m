...
#ifdef APP_HD
const float NJKInitialProgressValue = 0.9f;//0.1
const float NJKInteractiveProgressValue = 0.9f;//0.5
const float NJKFinalProgressValue = 0.95f;
#elif APP_PH
const float NJKInitialProgressValue = 0.6f;//0.1
const float NJKInteractiveProgressValue = 0.8f;//0.5
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if (ret && !isFragmentJump && isHTTP && isTopLevelNavigation) {
        ...
        if (temptProgress == 1.0) {
            
            [self reset];
            temptProgress = _progress;
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if ((complete && isNotRedirect) || !webView.loading) {
        [self completeProgress];
    }
}
...
- (void)webView:(UIWebView *)webView didCreateJavaScriptContext:(JSContext *)ctx {
    if ([_webViewProxyDelegate respondsToSelector:@selector(webView:didCreateJavaScriptContext:)]) {
        [_webViewProxyDelegate webView:webView didCreateJavaScriptContext:ctx];
    }
}
#endif
