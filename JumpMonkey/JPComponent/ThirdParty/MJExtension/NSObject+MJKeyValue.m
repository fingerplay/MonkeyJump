...
-(void)setTypeErrorDic:(NSMutableDictionary *)typeErrorDic{
    objc_setAssociatedObject(self, "typeErrorDic", typeErrorDic, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(NSMutableDictionary *)typeErrorDic{
    
    NSMutableDictionary *typeErrorDic = objc_getAssociatedObject(self, "typeErrorDic");
    if (!typeErrorDic) {
        typeErrorDic = [NSMutableDictionary dictionaryWithCapacity:1];
        objc_setAssociatedObject(self, "typeErrorDic", typeErrorDic, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return typeErrorDic;
}
...
#ifdef DEBUG
    NSMutableDictionary *typeError = [NSMutableDictionary dictionary];
#endif
        ...
        @try {
            ...
            if (!type.isFromFoundation && propertyClass) { // 模型属性
                ...
                if (propertyClass && ![value isKindOfClass:propertyClass]) {
                    ...
                    if (value) {
                        NSString *key = [NSString stringWithFormat:@"%@.%@",NSStringFromClass(property.srcClass),property.name];
                        [typeError setObject:value forKey:key];
                    }
#endif
                    value = nil;
                }
...
#ifdef DEBUG
    if (typeError && [typeError count]>0) {
        [self.typeErrorDic addEntriesFromDictionary:typeError];
    }
#endif
    ...
    [clazz mj_enumerateProperties:^(MJProperty *property, BOOL *stop) {
        @try {
            ...
            if ([value isKindOfClass:[NSObject class]] && [value superclass] == nil) {
                NSLog(@"这是一个错误的对象: %@",value);
                NSCAssert(NO, @"MJ解析进入防御代码");
                return;
            }
